...
async def upload_document(
    ...
    # Security 3: File Whitelist & Size Validation
    filename = file.filename or ""
    allowed_extensions = (".pdf", ".txt", ".eml", ".doc", ".docx")
    if not filename.lower().endswith(allowed_extensions):
        raise HTTPException(status_code=400, detail="Invalid file type. Only PDF, TXT, EML, DOC, and DOCX files are allowed.")

    content_bytes = await file.read()
    
    # Security 4: Enforce 5MB File Size Cap
